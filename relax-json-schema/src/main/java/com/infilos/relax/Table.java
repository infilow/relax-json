...
import com.infilos.relax.iterator.*;
import com.infilos.relax.source.*;
import com.infilos.relax.util.JsonUtil;
import com.infilos.relax.util.TableSchemaUtil;
...
/**
 * <pre>
 *  * This class represents a CSV or JSON-array encoded  table with optional CSV specification via a {@link CSVFormat} and optional provable assurances towards format integrity via a Table
 *  * {@link Schema}.
 *  *
 *  * This class makes a semantic difference between constructors and the overloaded factory method {@link #fromSource}. constructors are intended for capturing new data, while the `fromSource()` method is intended for reading existing CSV or
 *  * JSON data.
 *  *
 *  * Reading data from a Table instance is done via a {@link TableIterator}, which can be configured to return table rows as:
 *  *
 *  * - String arrays, 
 ...
 *  * - or in an "extended" form (parameter `extended` = true) that returns an Object array where the first entry is the row number, the second is a String array holding the headers, and the third is an Object array holding the row data.
 *  *
...
public class Table {
    ...
    /**
     * Constructor for a Table from String array input data and optionally a Schema. Data is parsed into a DataSourceFormat object
     *
    ...
    public Table(Collection<String[]> data, String[] headers, Schema schema) {
        this.dataSource = new StringArrayDataSource(data, headers);
        this.schema = schema;
        if (null != schema)
            validate();
    }
    ...
    /**
     * Create Table on an {@link java.io.InputStream} for reading both the CSV/JSON data and the table schema.
     *
    ...
    public static Table fromSource(InputStream dataSource, InputStream schema, CSVFormat format) throws Exception {
        ...
        table.dataSource = DataSource.create(dataSource);
    ...
    /**
     * Create Table from a {@link java.io.File} containing the CSV/JSON data and without either a Schema or a CSVFormat.
     *
     * @param dataSource InputStream for reading the data from
     * @throws Exception if either reading or parsing throws an Exception
     */
    public static Table fromSource(File dataSource, File basePath) throws Exception {
        Table table = new Table();
        table.dataSource = DataSource.create(dataSource, basePath);
        return table;
    }

    /**
     * Create Table from a {@link java.io.File} containing the CSV/JSON data and with  a Schema and a CSVFormat.
     *
     * @param dataSource InputStream for reading the data from
     * @param schema     InputStream for reading table schema from
     * @param format     The expected CSVFormat if dataSource is a CSV-containing InputStream; ignored for JSON data
    ...
    public static Table fromSource(URL dataSource) throws IOException {
        Table table = new Table();
        table.dataSource = DataSource.create(dataSource.openStream());
        return table;
    }
        ...
        try {
            if (null == schema) {
                return TableSchemaUtil
                    .createSchemaHeaderMapping(dataSource.headers(), dataSource.headers());
            } else {
                return TableSchemaUtil
                    .createSchemaHeaderMapping(dataSource.headers(), getDeclaredHeaders());
            }
        } catch (Exception ex) {
    ...
    /**
     * Returns either the headers in the order declared in the Schema or in the order found in the data if no Schema has been set. In the case where we don't have a Schema, the order is only well-defined for Tables read from CSV.
     *
     * If the input source is a JSON array of JSON objects, the order of columns is arbitrary, as JSON objects do not preserve key order:
     *
    ...
    public String[] getHeaders() throws Exception {
        ...
        return this.dataSource.headers();
    }
    ...
    /**
     * Write as CSV file, the `format` parameter decides on the CSV options. If it is null, then the file will be written as RFC 4180 compliant CSV
     *
     * @param out           the Writer to write to
     * @param format        the CSV format to use
     * @param sortedHeaders the header row names in the order in which data should be exported
     */
    //@Override
    ...
    /**
     * Write Table data to a provided {@link java.io.Writer} - the `dataFormat` parameter decides on the data format, either CSV or JSON.
     *
     * @param out        the Writer to write to
     * @param dataFormat the format to use, either CSV or JSON.
     */
    public void write(Writer out, DataSource.Format dataFormat) {
        try {
            if (dataFormat.equals(DataSource.Format.FORMAT_CSV)) {
                try {
                    String[] headers = null;
                    if (null != schema) {
            ...
            } else if (dataFormat.equals(DataSource.Format.FORMAT_JSON)) {
    ...
    /**
     * Write Table data as CSV to a provided {@link java.io.Writer}, the `format` parameter decides on the CSV options. If it is null, then the data will be written in the CSV defined by the CSV format of the Table
     *
     * @param out    the Writer to write to
     * @param format the CSV format to use
     */
    public void writeCsv(Writer out, CSVFormat format) {
        ...
        write(out, DataSource.Format.FORMAT_CSV);
        this.format = oldFormat;
    }

    /**
     * Write as CSV file, the `format` parameter decides on the CSV options. If it is null, then the data will be written in the CSV defined by the CSV format of the Table
     *
    ...
    /**
     * Validates that names of the headers are as declared in the Schema, and throws a TableValidationException if they aren't. If the headers derived from the DataSourceFormat aren't reliable (eg. JSON array of JSON objects where
     * properties that are `null` would be omitted), then we don't test whether all declared header names are present.
     *
     * Sort order is neglected to allow a Schema to define column order. This is intentional, as JSON-objects do not have a sort order of their keys. Therefore, reading not from a CSV but a JSON array of JSON objects needs this
     * flexibility.
     *
     * @throws TableValidationException thrown if the header names do not match the fields declared in the schema
    ...
    public Table setCsvFormat(CSVFormat format) {
        ...
        if ((null != dataSource) && (dataSource instanceof CsvDataSource)) {
            ((CsvDataSource) dataSource).setFormat(format);
        }
        return this;
    }
    ...
    /**
     * Set a Schema for this Table. If the Table is connected to a DataSourceFormat, ie. holds data, then the data will be validated against the new Schema.
     *
     ...
     */
    public DataSource getDataSourceFormat() {
        return this.dataSource;
    }

    /**
     * Set a DataSourceFormat for this Table, ie. set the data for the Table. If this Table has an active Schema, then the data will be validated against the Schema.
     *
     * @param fmt the DataSourceFormat to set
     */
    public Table setDataSourceFormat(DataSource fmt) {
        this.dataSource = fmt;
        if (null != schema)
            validate();
        return this;
