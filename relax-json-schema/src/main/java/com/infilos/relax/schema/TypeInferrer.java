...
import com.infilos.relax.util.MapValueComparator;
...
public class TypeInferrer {
    ...
    public static TypeInferrer instance() {
        return TypeInferrerHolder.INSTANCE;
    }

    private static final class TypeInferrerHolder {
        static final TypeInferrer INSTANCE = new TypeInferrer();
    }
     ...
     */
    public synchronized String infer(List<Object[]> data, String[] headers) throws TypeInferringException {
     ...
     * Infer the data types and return the generated schema.
     */
    public synchronized String infer(List<Object[]> data, String[] headers, int rowLimit) throws TypeInferringException {
        ...
        for (int rowIdx = 0; rowIdx <= rowLimit; rowIdx++) {
            ...
            for (int colIdx = 0; colIdx < row.length; colIdx++) {
                this.findType(headers[colIdx], row[colIdx].toString());
            }
        }
    ...
    /**
     * We use a map to keep track the inferred type counts for each field. Once we are done inferring, we settle for the type with that was inferred the most for the same field.
     */
